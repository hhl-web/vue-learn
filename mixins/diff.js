// createElemnt（h）函数将真实dom转换成=》vdom，虚拟dom有type,key,props，children,text属性。
// 比对：
// 1.判断type是否相等，不相等直接replaceChild替换。
// 2.静态数据
// 静态数据即是字符串，则只有text值，其他属性值都为underfined。也是会进行比对的，判断是否相等。
// 3.获取儿子，儿子比对
// 老的有儿子，新的也有儿子，要一一对比。
// 老头和新头比对
// 老尾和新尾对比
// 老头和新尾
// 老尾和新头
// 暴力比对，获取新的看看老的里面是否有，如果老的有，新没有，那个元素设置为underfined，新的有，老的没有，将元素插到老的第一个元素前面。

// 老的没有，新的有，在老的里面添加新的
// 老的有，新的没有，直接删除老的


// 并没有再新建新的真实dom元素。而是在原来老的dom元素在更新。
// key的作用是为了做比对，判断是否相等？？？？回答的太敷衍了
